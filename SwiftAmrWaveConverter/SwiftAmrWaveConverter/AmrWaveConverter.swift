...
fileprivate func waveHeaderData(sampleRate : UInt32, dataSize : UInt32) -> Data {
    ...
    var waveHeader = WaveHeader(header: ChunkHeader(ckID: riff, ckSize: dataSize + 36), ckFmt: wave)
    
    // fmt
    let fmt = ChunkDWord(c1: 102, c2: 109, c3: 116, c4: 32)
    let fmtHeader = ChunkHeader(ckID: fmt, ckSize: 16)
    var ckFmt = ChunkFmt(header: fmtHeader, audioFmt: 1, numChannels: 1, sampleRate: sampleRate, byteRate: sampleRate*2, blockAlign: 2, bitsPerSample: 16)
    
    // data
    let dataDWord = ChunkDWord(c1: 100, c2: 97, c3: 116, c4: 97)
    var dataHeader = ChunkHeader(ckID: dataDWord, ckSize: dataSize)
